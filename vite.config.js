            ...
            try {
                    ...
                    if (file.endsWith('.jsx') || file.endsWith('.tsx')) {
                        ...
                        for (const line of lines) {
                            ...
                            if (trimmed.startsWith('import') && trimmed.includes('from')) {
                                continue; // Skip ALL imports — local AND external
                            }
                            cleanedCode += line + '\n';
                        }
                ...
                for (const [local, info] of namespaceUsages) {
                    if (info.source === 'three') {
                        ...
                        // Always keep the namespace if it was imported as *
                ...
                // Convert other namespaces to named imports
                for (const [local, { source, used }] of namespaceUsages) {
                    if (source === 'three') continue; // already handled
                    if (used.size > 0) {
